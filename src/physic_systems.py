...
class GhostSystem(esper.Processor):
    ...
    # how long (seconds) a ghost chases the player once triggered
    FOLLOW_DURATION = 3.0
    # distance (world units) to trigger the chase
    PROXIMITY_THRESHOLD = 4.0

    def __init__(self):
        self.free_light_count = 0
        # map: ghost_entity_id → seconds of chase time remaining
        self.follow_timers = {}

    def process(self):
        for e_id, (ghost_col, report, velocity, animation, light, ghost) in \
            ...
            # 1) Hurt the player if collided
            if self.world.player_object in report.failed:
                self.world.damage_player()
            report.failed.clear()

            # 2) PROXIMITY‑BASED CHASE PHASE
            #    a) compute distance to player
            ghost_trans  = self.world.component_for_entity(e_id, com.Transformation)
            player_trans = self.world.component_for_entity(self.world.player_object, com.Transformation)
            dist = glm.length(player_trans.position - ghost_trans.position)

            #    b) if close and not already chasing, start the timer
            if self.follow_timers.get(e_id, 0.0) <= 0.0 and dist < self.PROXIMITY_THRESHOLD:
                self.follow_timers[e_id] = self.FOLLOW_DURATION

            #    c) if timer > 0, chase
            time_left = self.follow_timers.get(e_id, 0.0)
            if time_left > 0.0:
                direction = glm.normalize(player_trans.position - ghost_trans.position)
                velocity.value = glm.vec3(direction.x, direction.y, 0.0) * GhostSystem.SPEED
                # decrement chase timer
                self.follow_timers[e_id] = time_left - self.world.delta
                # skip wandering & light logic while chasing
                continue

            # 3) FALLBACK WANDER LOGIC (unchanged)
            if ghost_col.is_colliding_y or ghost_col.is_colliding_x or glm.length(velocity.value) < 1:
                velocity.value = glm.normalize(
                    glm.vec3(rand.uniform(-1.0, 1.0),
                             rand.uniform(-1.0, 1.0), 0.0)
                ) * GhostSystem.SPEED

            # 4) LIGHT & ANIMATION LOGIC (unchanged)
            if light.enabled:
